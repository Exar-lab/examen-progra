...
import com.buses.examen.Progra.customer.adapter.in.web.dto.response.AuthenticatedSessionResponse;
...
public class AuthenticationController {
    ...
    /**
     * Retorna la identidad del cliente autenticado en la sesión actual.
     *
     * @param authentication autenticación actual resuelta por Spring Security
     * @return datos mínimos de la sesión activa
     */
    @GetMapping("/me")
    public AuthenticatedSessionResponse me(final Authentication authentication) {
        final Object principal = authentication.getPrincipal();
        if (!(principal instanceof AuthenticatedCustomerPrincipal customerPrincipal)) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Sesión inválida");
        }
        return new AuthenticatedSessionResponse(customerPrincipal.clienteId(), customerPrincipal.getUsername());
    }
}
